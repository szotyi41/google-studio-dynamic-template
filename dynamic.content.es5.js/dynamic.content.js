...
function replaceContent(contentFields) {
  ...
  console.log("Dynamic Content loaded: ", contentFields); // Each in placeholders

  for (var i in placeholders) {
    placeholder = placeholders[i].toLowerCase();
    value = contentFields[placeholder]; // Import css file
    ...
    url = _typeof(value) === 'object' && typeof value.Url !== 'undefined' ? value.Url : value; // Create dom query selector

    selector = ["#" + placeholder, // Find for #background_image_1
    ...
    elements = document.querySelectorAll(selector); // If queried elements not found for placeholder
    ...
    for (var e = 0, element; element = elements[e]; e++) {
      try {
        ...
        if (placeholder.indexOf("url") !== -1) {
          url = value.Url ? value.Url : value; // If tag nam is <a> set href
          ...
          console.log(placeholder, "is a click url, set onclick event and Exit url to: ", url); // Set click url
        ...
        console.warn(placeholder, "placeholder not used (DOM not changed) value is (", _typeof(value), "): ", value);
      } catch (error) {
...
function callAPI(element, url) {
  ...
  try {
    var value; // Get parameters to show

    var urlParams = new URLSearchParams(url); // Get show parameters

    var showParam = urlParams.get('show') || ''; // Array of ['main', 'temp'] or ['current', '2', 'temp']
    ...
    xhttp.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
        ...
        if (_typeof(json) === 'object') {
          console.log('API data received', json); // Get the value (find) main.temp
